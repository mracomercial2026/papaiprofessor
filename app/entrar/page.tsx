...
export default function EntrarPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (err) {
      if (err.includes("Invalid login credentials")) setError("E-mail ou senha incorretos.");
      else if (err.includes("Email not confirmed"))  setError("Confirme seu e-mail antes de entrar.");
      else setError(err);
      return;
    }
      ...
      <div style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ ...glass, padding: "40px 36px" }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                <label style={{ fontFamily: SANS, fontSize: 13, fontWeight: 600, color: "#94a3b8" }}>
                  Senha
                </label>
                <button
                  type="button"
                  onClick={() => alert("Em breve! Por enquanto, entre em contato pelo Instagram.")}
                  style={{ background: "none", border: "none", fontFamily: SANS, fontSize: 12, color: "#6366f1", cursor: "pointer" }}
                >
                  Esqueci a senha
                </button>
              </div>
          ...
          <p style={{ fontFamily: SANS, fontSize: 13, color: "#475569", textAlign: "center", marginTop: 24 }}>
            Não tem conta ainda?{" "}
            <Link href="/cadastro" style={{ color: "#a78bfa", textDecoration: "none", fontWeight: 600 }}>
              Criar grátis
            </Link>
          </p>
        </div>
